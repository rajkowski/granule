...
public class Parser {
     ...
     *
     * @throws com.granule.json.JSONException Thrown if an error occurs in tokenizing the JSON string.
     */
        ...
        try {
            ...
            if (lastToken != Token.TokenBraceL)
                throw new JSONException(
                        "Expecting '{' " + tokenizer.onLineCol() + " instead, obtained token: '" + lastToken + "'");
            lastToken = tokenizer.next();

            while (true) {
                if (lastToken == Token.TokenEOF)
                    throw new JSONException("Unterminated object " + tokenizer.onLineCol());
                ...
                if (!lastToken.isString())
                    throw new JSONException("Expecting string key " + tokenizer.onLineCol());
                String key = lastToken.getString();

                lastToken = tokenizer.next();
                if (lastToken != Token.TokenColon)
                    throw new JSONException("Expecting colon " + tokenizer.onLineCol());
    ...
    public JSONArray parseArray(boolean ordered, JSONArray array) throws JSONException {
        ...
        try {
            if (lastToken != Token.TokenBrackL)
                throw new JSONException("Expecting '[' " + tokenizer.onLineCol());
            lastToken = tokenizer.next();
            while (true) {
                if (lastToken == Token.TokenEOF)
                    throw new JSONException("Unterminated array " + tokenizer.onLineCol());
    ...
    public Object parseValue(boolean ordered) throws JSONException {
        if (lastToken == Token.TokenEOF)
            throw new JSONException("Expecting property value " + tokenizer.onLineCol());
        ...
        try {
            ...
            if (lastToken == Token.TokenBrackL)
                return parseArray(ordered, null);
            if (lastToken == Token.TokenBraceL)
                return parseObject(ordered, null);
