...
public class LoggerFactory {
	...
	public static Logger getLogger(final String name) {
		if (defaultLogger == null)
			defaultLogger = determineDefaultLogger(name);
	...
	private static Logger determineDefaultLogger(String name) {
		...
		if (isClassAvailable("org.apache.commons.logging.Log")) {
			String logClassName = "";
			try {
				logClassName = Class.forName("org.apache.commons.logging.LogFactory").getMethod("getLog", String.class)
						.invoke(null, "test").getClass().getName();
			} catch (Exception e) { //
			}

			if (logClassName.equals("org.apache.commons.logging.impl.Jdk14Logger"))
				return new JavaLogger(name);
