...
public class CompressTagHandler {
    ...
    public String handleTag(IRequestProxy requestProxy, IRequestProxy runtimRequest, String oldBody)
                        ...
                        if (attrs.isValueExists("src")) {
                            ...
                            if (PathUtils.isWebAddress(src) || !PathUtils.isValidJs(src)) {
                                throw new JSCompileException(
                                        "Dynamic or remote scripts can not be combined. src=" + src);
                            }
                            ...
                            if (settings.isIgnoreMissedFiles() && !file.exists()) {
                                addFile = false;
                                logger.warn(MessageFormat.format("JS File {0} not found, ignored. Real Path={1}", path,
                                        requestProxy.getRealPath(path)));
                            }
                            if (addFile) {
                            ...
                            if (i != scripts.size() - 1) {
                                ...
                                if (fragmentDescriptors.size() > 0) {
                                    String newText = "<script type=\"text/javascript\" src=\""
                                            + PathUtils.getContextURL(runtimRequest.getContextPath(), "/combined.js?id="
                                                    + bundleId)
                                            + "\"></script>";
    ...
    private String processChunk(String chunk, IRequestProxy requestProxy, IRequestProxy runtimeRequest,
            ...
            for (int i = 0; i < links.size(); i++) {
                ...
                if (attrs.isValueExists("rel") && attrs.getValue("rel").equalsIgnoreCase("stylesheet")
                    ...
                    if (settings.isCleanCssDuplicates()
                        ...
                        eliminatedStyles.add(i);
                        logger.warn(MessageFormat.format("CSS File {0} not found, ignored. Real Path={1}", path,
                                requestProxy.getRealPath(path)));
                    } else {
