...
public class MiniParser {
        ...
        while (scanner.hasSymbols()) {
            ...
            if (scanner.hasSymbols()) {
                if (text.startsWith("<!--", scanner.getCurrentPosition()))
                    ...
                    processTag(s, scanner, finishTags);
            }
        }
    }
    ...
    private void processTag(TagReader s, Scanner scanner, List<String> finishTags) {
        ...
        //attributes
        while (scanner.getCurrentPosition() < text.length()
                && Character.isLetter(text.charAt(scanner.getCurrentPosition()))) {
            ...
            if (scanner.getCurrentPosition() < text.length() && text.charAt(scanner.getCurrentPosition()) == '=') {
                ...
                if (scanner.getCurrentPosition() < text.length()) {
                    String value = "";
                    if (text.charAt(scanner.getCurrentPosition()) == '"'
                            || text.charAt(scanner.getCurrentPosition()) == '\'')
                        value = scanner.getQuotedAttribute(text.charAt(scanner.getCurrentPosition()));
                    else
                        value = scanner.getNotQuotedAttribute();
                    a.setValue(value);
                    a.setEnd(scanner.getCurrentPosition());
                }
            }
        ...
        if (scanner.getCurrentPosition() < text.length()) {
            if (text.startsWith("/>", scanner.getCurrentPosition())) {
                ...
                t.setEnd(scanner.getCurrentPosition());
                if (t.getName() != null
                        && (t.getName().equalsIgnoreCase("script") || t.getName().equalsIgnoreCase("style"))) {
